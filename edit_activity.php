...
//						alert(data);
			...
			//submit new activity 
			function submit_update_activity(){
...
//				}
				...
				if(!window.confirm("确定要更新活动信息嘛?")){
					return;
				}
				
				var orgnizer = $("#weixin_id").val();
				
				var params = "aid="+$("#aid").val()+"&theme="+theme+"&activity_time="+activity_time+"&deadline_time="+deadline_time+"&location="+activity_loc+"&detail="+detail+"&phone="+phone;
				var url = "core/controller/ActivityController.php?action=updateActivity&"+params;
...
//						alert(data);
						...
						if(obj.ret_code == "200"){
							alert("活动信息已更新....");
							document.location.href="activity_detail.php?weixin_id="+$("#weixin_id").val()+"&aid="+$("#aid").val()+"&type=detail";
						}else{
							alert("发起活动失败...请稍后再试");
						}
					}
		...
		function backToDetail(){
		  		...
		  		var aid = $("#aid").val();
				document.location.href = "activity_detail.php?weixin_id="+weixin_id+"&aid="+aid;
		  ...
		  function validatePhone(){
			...
			function query_activity(){
				...
				var aid = $("#aid").val();
				var url = "core/controller/ActivityController.php?action=queryActivity&aid="+aid;
...
//						alert(data);
						...
						if(obj.ret_code == "200"){
							var activity = obj.ret_res[0];
							$("#theme").val(activity.theme);
							$("#activity_time").val(activity.activity_time);
							$("#deadline_time").val(activity.deadline_time);
							$("#location").val(activity.location);
							$("#detail").val(activity.detail);
							$("#orgnizer").val(activity.orgnizer);
							$("#phone").val(activity.phone);
							textCounter('theme','theme_count',10);
							textCounter('detail','detail_count',240);
						}
					}
				});
	...
	<body>
		<header class="mui-bar mui-bar-nav">
			...
			<button class="mui-btn mui-pull-right" onclick="submit_update_activity()">
				提交修改
			</button>
			<h1 class="mui-title">
				修改活动
