...
//						alert(data);
			...
			function append_activity_info(obj){
				...
				var dom = "<ul class=\"mui-table-view\">";
					...
					if(obj.status == '0'){
						dom +="<button class=\"mui-btn mui-btn-primary mui-btn-yellow\" height=\"40px\" onclick=\"document.location.href='activity_detail_admin.php?weixin_id="+selfId+"&aid="+obj.id+"&type=detail'\">已取消</button>";
					}else if(obj.orgnizer==selfId || obj.signup == "1" || lastTime == '报名已截止'){
						dom +="<button class=\"mui-btn mui-btn-primary mui-btn-primary\" height=\"40px\" onclick=\"document.location.href='activity_detail_admin.php?weixin_id="+selfId+"&aid="+obj.id+"&type=detail'\">详 情</button>";
				    }else{
						dom += "<button class=\"mui-btn mui-btn-primary mui-btn-primary\" height=\"40px\" onclick=\"document.location.href='activity_detail_admin.php?weixin_id="+selfId+"&aid="+obj.id+"&type=detail'\">详 情</button></li>";
	                }
	                dom += "</ul>";
...
//						alert(data);
			...
			function logout(){
				...
				var url = "../core/controller/AdminController.php?action=logout";
//				alert(url);
				$.ajax({
					type:"get",
					url: url,
...
//						alert(data);
	...
	<body>
		<header class="mui-bar mui-bar-nav">
			<button class="mui-btn mui-pull-left" onclick="logout()">
				退 出
			</button>
			<h1 class="mui-title">
				发布了的活动
